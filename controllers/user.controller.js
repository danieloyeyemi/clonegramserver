...
const comment = (req, res) => {
    const user = req.body.commentId
    picSchema.findOne({userId:user},(err,result)=>{
        if(err) {
            console.log(err);
            res.send({message:'Can not push'})
        }
        else{
            console.log(result);
            res.send({message:'pushed successfully',comments:'yes it worked'})
        }
    })
}
...
module.exports = { registerUser, login, getDashboard, UploadFile, profile, setProfile, getPost, getAllUser, comment }